...
def extract_notices(html: str, base_url: str = LIST_URL) -> list[Notice]:
    ...
    # Preferred path: normal anchors generated by the page after JS rendering.
    for a in soup.find_all("a", href=True):
        ...
        if not title:
            # Some pages put the title in a parent/card element.
    ...
    # Fallback: if the detail URL appears in scripts/attributes but not a normal anchor.
    if not found:
...
def fetch_rendered(url: str) -> str:
    ...
    GitHub's ubuntu-latest runner already ships with Chrome. Selenium Manager can
    resolve the matching driver automatically in most runs.
    """
    ...
    try:
        ...
        try:
            ...
            # Give slow XHR a short final chance; page_source is still useful for
            # diagnostics even when the wait condition did not fire.
            time.sleep(3)
...
def collect_notices() -> list[Notice]:
    ...
    # Cheap first pass: static HTML from both the dedicated list page and homepage.
    ...
    # The official list is JS-driven on some deployments. If static parsing did not
    # reveal any detail URLs, fall back to a rendered browser page.
    if not notices:
        try:
    ...
    # Keep the most relevant notices first for cleaner logs.
...
def load_state() -> dict:
    if not STATE_PATH.exists():
        return {"initialized": False, "seen_ids": [], "last_checked_utc": None}
    try:
...
if __name__ == "__main__":
    try:
        ...
        # A hard failure is intentional: GitHub can also notify on workflow failures,
        # and it prevents silent breakage if the official site changes its HTML.
