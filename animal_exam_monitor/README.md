...
另外，GitHub 对长期无仓库活动的公开仓库可能自动停用 scheduled workflow。本项目每月 1 日会自动更新一次 `.keepalive` 标记并提交，避免因为几个月都没有新考试通知而让监控悄悄停掉。
...
- `state.json` 只在基线建立或出现新公告时更新，避免每分钟制造无意义提交。
- 单个通知渠道失败不会阻止其他渠道继续报警，也不会因为状态未保存而无限重复发信。
- 每月 keepalive commit 防止长期无仓库活动导致定时任务被 GitHub 自动停用。
